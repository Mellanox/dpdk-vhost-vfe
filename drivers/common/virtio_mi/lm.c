	...
	*head = virtio_vdpa_send_admin_command_split(avq, ctrl, dat_ctrl,
	...
	for (i = 0;; i++) {
		...
		if(ctrl->status && (!(ctrl->status & VIRTIO_ADMIN_CMD_STATUS_DNR_BIT))) {
			DRV_LOG(DEBUG, "No:%d cmd status:0x%x, submit again after 1s", i, ctrl->status);
			usleep(1000000);
			/* Kick again */
		...
		struct rte_pci_device *pci_dev)
{
	...
	priv->vpdev = virtio_pci_dev_alloc(pci_dev, device_fd, false);
	if (priv->vpdev == NULL) {
		DRV_LOG(ERR, "%s failed to alloc virtio pci dev", devname);
		ret = rte_errno ? -rte_errno : -VFE_VDPA_ERR_DEVICE_PROBE_FAIL;
		goto error;
	}
...
static void
virtio_ha_pf_drv_ctx_set(const struct virtio_dev_name *pf, const void *ctx, __rte_unused struct virtio_ha_vm_dev_ctx *vm_ctx)
{
